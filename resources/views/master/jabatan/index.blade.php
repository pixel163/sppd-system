...
<div class="p-6" x-data="{ openCreate: false, openEdit: false, editId: null, editName: '', editActive: true }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Master Data Jabatan</h1>
            <p class="text-sm text-slate-500">Kelola daftar jabatan perjalanan dinas</p>
        </div>
            ...
            <tbody class="divide-y divide-slate-200 text-sm">
                @forelse($jabatans as $index => $item)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 text-slate-500">{{ $jabatans->firstItem() + $index }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <!-- Tombol Hapus -->
                                <form action="{{ route('master.jabatan.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-slate-400">Belum ada data jabatan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="openCreate" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" @click.away="openCreate = false">
            <h2 class="text-lg font-bold text-slate-800 mb-4">Tambah Jabatan Baru</h2>
    ...
    <div x-show="openEdit" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" @click.away="openEdit = false">
            <h2 class="text-lg font-bold text-slate-800 mb-4">Edit Jabatan</h2>
            <form :action="'{{ url('master/jabatan') }}/' + editId" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
