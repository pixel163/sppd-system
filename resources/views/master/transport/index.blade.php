...
<div class="p-6" x-data="{ openCreate: false, openEdit: false, editId: null, editName: '', editActive: true }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Master Data Transport</h1>
            <p class="text-sm text-slate-500">Kelola daftar transport perjalanan dinas</p>
        </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    ...
                    <th class="px-6 py-4">Nama Transportasi</th>
            ...
            <tbody class="divide-y divide-slate-200 text-sm">
                @forelse($transports as $index => $item)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 text-slate-500">{{ $transports->firstItem() + $index }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <!-- Tombol Hapus -->
                                <form action="{{ route('master.transport.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-slate-400">Belum ada data transport.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="openCreate" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" @click.away="openCreate = false">
            <h2 class="text-lg font-bold text-slate-800 mb-4">Tambah Transport Baru</h2>
            <form action="{{ route('master.transport.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Nama Transport</label>
    ...
    <div x-show="openEdit" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" @click.away="openEdit = false">
            <h2 class="text-lg font-bold text-slate-800 mb-4">Edit Transport</h2>
            <form :action="'{{ url('master/transport') }}/' + editId" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
